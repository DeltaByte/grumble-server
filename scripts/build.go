...
func main() {
	...
	flag.BoolVar(&printGenVersion, "gen-version", printGenVersion, "generate Grumble version and output (default: false)")
	flag.Parse()
	...
	if flag.NArg() == 0 {
		log.Println("Usage: go run scripts/build.go build")
		return
	}
	...
	for _, cmd := range flag.Args() {
		...
		case "build":
			clean()
			doBuild("grumble-server", "./cmd/grumble-server", buildTags)
...
func makeLatestDistCopies() {
	...
	latestMapping := map[string]string{
		"_amd64.deb":               "dist/grumble_latest_amd64.deb",
		".x86_64.rpm":              "dist/grumble-latest-1.x86_64.rpm",
		".linux-amd64.tar.gz":      "dist/grumble-latest.linux-x64.tar.gz",
		".linux-amd64-musl.tar.gz": "dist/grumble-latest.linux-x64-musl.tar.gz",
		".linux-armv7.tar.gz":      "dist/grumble-latest.linux-armv7.tar.gz",
		".linux-armv7-musl.tar.gz": "dist/grumble-latest.linux-armv7-musl.tar.gz",
		".linux-armv6.tar.gz":      "dist/grumble-latest.linux-armv6.tar.gz",
		".linux-arm64.tar.gz":      "dist/grumble-latest.linux-arm64.tar.gz",
		".linux-arm64-musl.tar.gz": "dist/grumble-latest.linux-arm64-musl.tar.gz",
	}
...
func readVersionFromFile() {
	rawVersion, err := ioutil.ReadFile("VERSION")
	if err != nil {
		log.Fatal("Failed to open VERSION file")
		return
	}

	pattern := `^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-((?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*)(?:\.(?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*))*))?(?:\+([0-9a-zA-Z-]+(?:\.[0-9a-zA-Z-]+)*))?$`
	if matched, err := regexp.Match(pattern, rawVersion); err != nil && matched == true {
		log.Fatal("Failed to decode version")
	}
